...
def main():
    ...
    if not args.reset and mngr.latest_step() is not None:
        ...
        with mesh:
            # 1. Capture the "pristine" states BEFORE Orbax touches them.
            # These contain the perfect 8-core shardings generated by your dummy forward pass.
            ...
            # 2. THE FIX: Force Orbax's restored arrays back into the pristine shardings!
            # This catches the dropped PRNG keys and scalars and puts them back on all 8 cores.
            restored_model_sharded = jax.tree.map(
                lambda r, p: jax.device_put(r, p.sharding) if hasattr(p, 'sharding') else r,
                restored['model'],
                pristine_model_state
            )

            restored_opt_sharded = jax.tree.map(
                lambda r, p: jax.device_put(r, p.sharding) if hasattr(p, 'sharding') else r,
                restored['optimizer'],
                pristine_opt_state
            )

            # 3. Inject the correctly sharded arrays back into our stateful NNX objects
            nnx.update(model, restored_model_sharded)
            nnx.update(optimizer, restored_opt_sharded)

            start_step = restored['step']

            # 4. Re-split after restoring so the training loop gets the updated, fully sharded state!
            graphdef, state = nnx.split((model, optimizer))
